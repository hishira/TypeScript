...
use crate::api::dtos::userdto::userdto::{CreateUserDto, UserFilterOption};
...
impl UserService {
    ...
    pub async fn create_managed_user(&self, params: CreateUserDto, owner_id: Uuid ) -> Result<Json<bool>, ResponseError> {
        let user = UserUtils::get_user_from_dto(UserDtos::Create(params), None).await?;
        let ids_touples = (owner_id, user.id.get_id());
        self.user_repo.create(user).await;
        let result =
            self.user_dao.create_user_connection(ids_touples).await;
        match result {
            Ok(_) => Ok(Json(true)),
            Err(error) => {
