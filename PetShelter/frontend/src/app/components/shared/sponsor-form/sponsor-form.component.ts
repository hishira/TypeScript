...
export class SponsorFormComponent implements OnInit {
  @Output() formEmitter: EventEmitter<PetsponsorValue> =
    new EventEmitter<PetsponsorValue>();
  ...
  ngOnInit(): void {
    this.form = this.formBuilder.group<PetSponsorForm>({
      amount: new FormControl(DEFAULT_DONATE_COST, {
        nonNullable: true,
      ...
      confirmEmail: new FormControl('', {
        ...
        validators: [
          Validators.required,
          Validators.email,
          this.validatorService.emailMath('email'),
        ],
      }),
